...
class AlipaySmController extends PayController{
    ...
    public function Pay($array){
        ...
        $notifyurl = $this->_site . 'Pay_AlipaySm_notifyurl.html';
        ...
        $callbackurl = $this->_site . 'Pay_AlipaySm_callbackurl.html?order_id='.$return['orderid'];
        //如果生成错误，自动跳转错误页面
   ...
   public function callbackurl(){
        file_put_contents('AlipaySmbackurl.txt',"order_id=".$_REQUEST["order_id"]."\r\n\r\n",FILE_APPEND);
        $Order = M("Order");
        $pay_status = $Order->where("pay_orderid = '".$_REQUEST["order_id"]."'")->getField("pay_status");

        if($pay_status <> 0){
      
            $this->EditMoney($_REQUEST["order_id"], 'AlipaySm', 1);
        }else{
            exit("error");
        }
    }
    ...
    public function notifyurl(){
        ...
        file_put_contents('A.txt',json_encode($_POST),FILE_APPEND);

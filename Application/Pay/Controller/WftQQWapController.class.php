...
class WftQQWapController extends PayController{
	...
	public function Pay($array){
		...
		$notifyurl = $this->_site . 'Pay_WftQQWap_notifyurl.html';

		$callbackurl = $this->_site . 'Pay_WftQQWap_callbackurl.html';
        ...
        if($respJson['status'] == '0' && $respJson['result_code'] == '0'){
            $sign_array = $respJson;
    ...
    public function notifyurl(){
        ...
        if($data['status'] == 0 && $respSign == $sign){
            $this->EditMoney($data["out_trade_no"], 'WftQQWap', 0);
            exit('success');
        }

...
class XinzhifuController extends PayController{
    ...
    public function Pay($array){
        ...
        $notifyurl = $this->_site . 'Pay_Xinzhifu_notifyurl.html';
        $parameter = array(
            'code' => 'Xinzhifu',
            'title' => '收银台支付（星支付wap）',
            'exchange' => 1, // 金额比例
        ...
        $callbackurl = $this->_site . 'Pay_Xinzhifu_callbackurl.html?order_id='.$return['orderid'];
        //如果生成错误，自动跳转错误页面
    ...
    public function callbackurl(){
        file_put_contents('Xinzhifucallbackurl.txt',"order_id=".$_REQUEST["sdorderno"]."\r\n\r\n",FILE_APPEND);
        $Order = M("Order");
        $pay_status = $Order->where("pay_orderid = '".$_REQUEST["sdorderno"]."'")->getField("pay_status");
        if($pay_status <> 0){
            $this->EditMoney($_REQUEST["sdorderno"], 'Xinzhifu', 1);
        }else{
            exit("error");
        }
    }

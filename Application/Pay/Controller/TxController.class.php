...
class TxController extends Controller
{
    ...
    public function draw()
    {
        ...
        $key = $_REQUEST['key'];    //md5(md5(paypassword).$money.$apikey)

        if (!$key || !$mch_id || !$trade_order_id || !$notifyurl || !$username || !$money || !$bankname || !$bankzhiname || !$banknumber || !$sheng || !$shi || !$bankfullname) {
        ...
        //个人信息
        $Member = M('Member');
        $mch_id2 = intval($mch_id)-10000;
        $info = $Member->where(['username' => $username, 'id' => $mch_id2])->find();
        if (!$info) exit(json_encode(['status' => 0, 'msg' => '商户不存在'], JSON_UNESCAPED_UNICODE));

        $my_key = md5($info['paypassword'].$money.$info['apikey'].$username.$mch_id.$bankname.$bankzhiname.$banknumber.$sheng.$shi.$bankfullname.$notifyurl.$trade_order_id);
    ...
    public function balance(){
        $username = $_REQUEST['username'];
        $mch_id = $_REQUEST['mch_id'];
        $key= $_REQUEST['key'];

        if(!$username || !$mch_id || !$key) exit(json_encode(['status' => 0, 'msg' => '参数不全'], JSON_UNESCAPED_UNICODE));
        ...
        //支付密码
        $my_key = md5($info['paypassword'].$info['apikey'].$username.$mch_id);
        if($key!=$my_key){
            exit(json_encode(['status' => 0, 'msg' => '验签key失败'], JSON_UNESCAPED_UNICODE));
        }
    ...
    public function qxt(){
        ...
        if(!$username || !$mch_id || !$trade_order_id || !$key) exit(json_encode(['status' => 0, 'msg' => '参数不全'], JSON_UNESCAPED_UNICODE));
        ...
        //支付密码
        $my_key = md5($info['paypassword'].$info['apikey'].$username.$mch_id.$trade_order_id); 
        //dump($info['paypassword'].$info['apikey'].$username.$mch_id.$trade_order_id);
        if($key!=$my_key){
            exit(json_encode(['status' => 0, 'msg' => '验签key失败'], JSON_UNESCAPED_UNICODE));
        }

...
class NMwxsmpayController extends PayController{

    protected $types;

    public function __construct(){
        parent::__construct();
        $this->types = 'NMwxsmpay';
    }
    ...
    public function Pay($array){
     ...
     /*   if(empty($bankname)){
        ...
        $arraystr['pay_productname'] =  '实物_数码家电';
        if( $return['pay_memberid'] == 10112 /*|| $return['pay_memberid'] == 10106*/ ){
            $arraystr['pay_type'] =  1;
        ...
        if( $arraystr['pay_type'] == 1){
             echo $url;die;
        }else{

            $d =   json_decode($url,true);
            if($d['code'] == 0){
                $d['orderid'] = $return['orderid'];

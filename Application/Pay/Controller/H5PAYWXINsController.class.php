...
class H5PAYWXINsController extends PayController{
    ...
    public function Pay($array){
       ...
       //$return["callbackurl"] ||  $return["callbackurl"] = $callbackurl;
        ...
        $arraystr = array(
            ...
            "in_cust_id" =>'',
            "in_acct_id" => '',
            "return_url" => $return["pay_callbackurl"]?$return["pay_callbackurl"]:$callbackurl,
            "bankno" => '',
        );

        $url =  "body=商品&mch_id=".$return["mch_id"]."&notify_url=".$return['notifyurl']."&out_trade_no=".$arraystr['out_trade_no']."&return_url=".$arraystr['return_url']."&service=".$arraystr['service']."&sign_type=MD5&total_fee=".$arraystr['total_fee']."&key=".$return['signkey'];
        //签名.
        $sign	= MD5($url);
        $arraystr['sign']=$sign;
/*        echo '<pre>';
    ...
    function request_post($url = '', $param = '') {
        ...
        $data['out_trade_id'] = $data['orderNo'];
        $data['datetime'] = date('Y-n-d H:i;s',time());
        $data['subject'] = '商品';

        $this->showQRcode($data['payinfo'], $data, $view = 'weixin');

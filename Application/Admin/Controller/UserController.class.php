...
class UserController extends BaseController
{
 ...
 /*
        ...
        //用户
        $userprods = M('for_user')->where(['userid' => 112])->find();
        $pids = [];
        if($userprods['polling']==0){
            $pids[] = $userprods['channel'];
        }else{
            $weights=array();
            $weights    = explode('|',  $userprods['weight']);
            ...
            if (is_array($weights)) {
                ...
                list($pid, $weight) = explode(':',$userprods['weight']);
                $pids[]= $pid;
            }
        ...
        $products = M('pay_for_another')->select();
    ...
    public function saveQuery(){
        ...
        $userid = explode('_',$userid['products']);
        if(empty($userid)){
             return;
        }

        $channel = M('pay_for_another')->where(['code' => $userid[0]])->find();
        ...
        $money = 0.00;
        if($userid[0] == 'Xdfpay' || $userid[0] == 'WLPay'|| $userid[0] == 'YYpayDf'|| $userid[0] == 'TxbPay'){
            $money = '暂无查询功能';
        }else{

            $money = R('Payment/'.$userid[0].'/moneyQuery');

            $money = sprintf("%.2f",$money);
        }

        echo json_encode(array('code'=>1,'money'=>$money,'title'=>$userid[1]));
    ...
    //编辑用户代付通道
    public function editUserFor()
    {
        //通道
        $channels = M('pay_for_another')
            ->where(['status' => 1])
            ->field('id,code,title,status')
            ->select();
        foreach ($channels as  $key=>$v){
            $channels[$key]['paytype'] = 1;
        }
        ...
        $products[0]['title'] = 1;
        $products[0]['status'] = 1;
        $products[0]['paytype'] = 1;

        //用户
        $userprods = M('for_user')->where(['userid' => $userid])->select();
        if ($userprods) {
        ...
        if ($products) {
            foreach ($products as $key => $item) {
                ...
                $products[$key]['is_zd'] = $_tmpData[$item['id']]['is_zd'];
                //权重
                $weights    = [];
    ...
    public function saveForUser()
    {
        if (IS_POST) {
            ...
            foreach ($u as $key => $item) {
                ...
                $product = M('for_user')->where(['userid' => $userid, 'pid' => $key])->find();

                if ($product) {

                    $data_insert[] = ['id' => $product['id'], 'userid' => $userid, 'pid' => $key, 'status' => $status, 'polling' => $item['polling'], 'is_zd' => $is_zd,'channel' => $item['channel'], 'weight' => trim($weightStr, '|')];
                } else {

                    $data_update[] = ['userid' => $userid, 'pid' => $key, 'status' => $status, 'polling' => $item['polling'],'is_zd' => $is_zd, 'channel' => $item['channel'], 'weight' => trim($weightStr, '|')];
                }

            }

            M('for_user')->addAll($data_insert, [], true);
            M('for_user')->addAll($data_update, [], true);
    ...
    public function editUser()
    {
        ...
        if ($userid) {
            $data = M('Member')
                ->where(['id' => $userid])->find();
            if($data['parentid'] !=1){
                $data['parentid']  = $data['parentid']+10000;
            }else{
                $data['parentid']  = '';
            }
        ...
        $mobile = M('websiteconfig')->field('id,mobile')->find();
        /**
         * 升级，用户组不再与用户组关联
         * author: feng
        ...
        $this->assign('sendUrl', U('Admin/User/upDfSend'));
        $this->display();
    }
     ...
     * 修改代付ip
     */
    public function upDfSend()
    {
        $mobile = I('request.mobile');

        $info = C('SMS_GD');
        $time=date('YmdHis',time());//时间戳，系统当前时间字符串，年月日时分秒
        $md5=md5($info['name'].$info['psw'].$time);//MD5加密，账号+密码+时间戳
        $str1='【糯米】你正在进行修改代付ip绑定操作，请保存好验证码:';//短信内容
        $res = $this->sendNew( 'upDfSend',$info['id'],$time,$md5,$mobile,$str1);
        $this->ajaxReturn(['status' => $res['code']]);

/*
        $res = $this->send('upDfSend', $mobile, '修改代付ip',5);
        $this->ajaxReturn(['status' => $res['code']]);*/
    }
    ...
    public function saveUser()
    {
        ...
        if (IS_POST) {
            ...
            if($_POST['send_st']==1){
                //校验验证码
                $code =  $_POST['code'];
                if (session('send.upDfSend') == $code && $this->checkSessionTime('upDfSend', $code)) {
                    session('send', null);
                } else {
            ...
            if(empty($u['df_ip'])){
                $this->ajaxReturn(['status' => 0, 'msg' => '代付IP报备不能为空']);
            }
            if ($userid) {
    ...
    public function userRateEdit()
    {
        ...
        if ($products) {
            foreach ($products as $key => $item) {
                $products[$key]['feilv']    = $_tmpData[$item['id']]['feilv']*100 ? $_tmpData[$item['id']]['feilv']*100 : '0';
                $products[$key]['fengding'] = $_tmpData[$item['id']]['fengding']*100 ? $_tmpData[$item['id']]['fengding']*100 : '0';
            }
        }
        $this->assign('products', $products);
        $this->display();
    ...
    public function saveUserRate()
    {
        if (IS_POST) {
            ...
            foreach ($rows as $key => $item) {
                if($item['feilv']> 0){
                    $item['feilv'] = $item['feilv']/100;
                }
                if($item['fengding']> 0){
                    $item['fengding'] = $item['fengding']/100;
                }
    ...
    public function saveUserProduct()
    {
        if (IS_POST) {
            ...
            foreach ($u as $key => $item) {
                ...
                if ($product) {
                    $data_insert[] = ['id' => $product['id'], 'userid' => $userid, 'pid' => $key, 'status' => $status, 'polling' => $item['polling'],'use_pid' => '', 'channel' => $item['channel'], 'weight' => trim($weightStr, '|')];
                } else {
                    $data_update[] = ['userid' => $userid, 'pid' => $key, 'status' => $status, 'polling' => $item['polling'], 'channel' => $item['channel'],'use_pid' => '', 'weight' => trim($weightStr, '|')];
                }
            }

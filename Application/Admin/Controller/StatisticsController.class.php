...
class StatisticsController extends BaseController
{
    ...
    public function userFinance()
    {
        ...
        if ($memberList) {
            ...
            foreach ($memberList as $k => $v) {
                ...
                //实付金额
                $memberList[$k]['paid_order_amount'] = $Order->where(['pay_memberid' => $payMemberid, 'pay_status'=>['in', '1,2']])->sum('pay_actualamount');
    ...
    public function platformReport() {
        ...
        if ($data) {
            foreach ($data as $k => $v) {
                ...
                $data[$k]['pay_profit'] = $tkmoney1 + $tkmoney2 - $money1 - $money2;
                //冻结金额
   ...
   */
    private function getDateBalance($userid, $date) {
        ...
        $log = M('Moneychange')->where(['userid'=>$userid, 'datetime'=>array('elt', $date), 't'=>['neq', 1], 'lx' => ['not in', '3,4']])->order('datetime DESC')->find();
        if(empty($log)) {
            $money = 0;
        } else {

...
class WithdrawalController extends BaseController
{
   ...
   public function tixing(){
        $is=M('Tklist')->where(array('is_new'=>0))->find();
        $is ? $this->success($is):$this->error($is);
    }
    ...
    public function index()
    {
       ...
       M('Tklist')->where(array('is_new'=>0))->save(array('is_new'=>1));
   ...
   public function payment()
   {
       ...
       if(UID==7){
           $wheres['id'] = array('eq', 17);
           $df_list = M('PayForAnother')->where($wheres)->select();
       }else{
       ...
       if(UID==7) {
        ...
        if(!empty($list)){
             $array=array();
            foreach ($pfa_lists as $r=>$t){
                $array[$t['id']] =$t;
            }
            $for_user = M('for_user')->select();

            foreach($list as $k => $v) {
                $str = $v['banknumber'];
                $list[$k]['banknumber'] =  chunk_split($str,4," ");
                if($v['t'] == 0 || $v['t'] == 1){
                   $list[$k]['t'] = 'D+'.$v['t'];
                }
                if($v['t'] == 7 || $v['t'] == 30){
                ...
                $list[$k]['tkmoney'] =  sprintf("%.2f",$v['tkmoney']);;
                $list[$k]['sxfmoney'] =  sprintf("%.2f",$v['sxfmoney']);;
                $list[$k]['money'] =  sprintf("%.2f",$v['money']);
                $list[$k]['allmoney'] =  $list[$k]['sxfmoney']+$list[$k]['money'];
                $list[$k]['df_qudao'] =  array();
                if($for_user){
                    foreach($for_user as $ks => $vs) {
                       if($v['userid'] == $vs['userid']) {
                           if($vs['polling'] == 0){
                               if(!empty($array[$vs['channel']])){
                                   //单渠道的
                                   $list[$k]['df_qudao'] = array($array[$vs['channel']]) ;
                               }

                           }else{
                               $weights  = explode('|', $vs['weight']);
                               if(is_array($weights)) {
                                   foreach ($weights as $value) {
                                       list($pid, $weight) = explode(':', $value);
                                       if ($pid) {
                                           $_tmpWeight[$pid] = ['channel' => $pid];
                                       }
                                   }
                               }

                               foreach ($_tmpWeight as  $kv=>$s){
                                   if(!empty($array[$kv])){
                                       $list[$k]['df_qudao'][] = $array[$kv];
                                   }
        ...
        if (IS_POST) {
            ...
            switch ($status) {
                case '2':
                    ...
                    //回调
                    $return_array['trade_order_id'] = $withdraw['trade_order_id'];
                    $return_array['status'] = 2;    //已处理
                    $return_array['code'] = '101';    //已处理
                    $notifystr = "";
                    ...
                    curl_setopt($ch, CURLOPT_URL, $withdraw["notifyurl"]);
    ...
    public function editwtStatus()
    {
        ...
        if (IS_POST) {
            $status  = I("post.status", 0, 'intval');
            $userid  = I('post.userid', 0, 'intval');
            $tkmoney = I('post.tkmoney')+I('post.sxfmoney');
            if (!$id) {
                $this->ajaxReturn(['status' => 0, 'msg' => '操作失败']);
            }
            //开启事物
            ...
            switch ($status) {
                ...
                case '3':
                    ...
                    if( $withdraw['code'] =='AApayDf'){
                        $map['_string'] = 'status=0 OR status=4  OR status=1 ';
                    }
            ...
            foreach ($ids as $k => $v) {
                try {
                    ...
                    if (intval($v)) {
                        ...
                        if($withdraw['status'] == 1){//提款申请处理中，不能驳回
                            $fail++;
                            continue;
                        } elseif($withdraw['status'] == 2) {//提款申请已打款，不能驳回
                            $fail++;
                            continue;
                        } elseif($withdraw['status'] == 3) {//提款申请已驳回，不能驳回
                            $fail++;
                            continue;
                        }
                        $map['status'] = 0;
    ...
    public function submitDfSend()
    {
        ...
        $res = $this->send('submitDfSend', $mobile ,'提交代付');

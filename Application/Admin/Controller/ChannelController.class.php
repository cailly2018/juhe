...
class ChannelController extends BaseController
{
    ...
    public function index()
    {
      ...
      if($data){
          ...
          foreach ($data as $key=>$v){
              if($v['defaultrate']>0){
                  $data[$key]['defaultrate']= sprintf("%.2f",$v['defaultrate']*100) ;
              }else{
                  $data[$key]['defaultrate']= '0.00';
              }
              if($v['fengding']>0){
                  $data[$key]['fengding']= sprintf("%.2f",$v['fengding']*100) ;

              }else{
                  $data[$key]['fengding']= '0.00';
              }
              if($v['rate']>0){
                  $data[$key]['rate']= sprintf("%.2f",$v['rate']*100) ;
    ...
    public function saveEditSupplier()
    {
        if (IS_POST) {
            ...
            $_request['fengding']     = $papiacc['fengding']? $papiacc['fengding']/100 : 0;
    ...
    public function editSupplier()
    {
        ...
        if ($pid) {
            ...
            if($pa['defaultrate']>0){
                $pa['defaultrate']= sprintf("%.2f",$pa['defaultrate']*100);
            }else{
                $pa['defaultrate']= '0.00';
            }
            if($pa['fengding']>0){
                $pa['fengding']= sprintf("%.2f",$pa['fengding']*100) ;

            }else{
                $pa['fengding']= '0.00';
            }
            if($pa['rate']>0){
                $pa['rate']= sprintf("%.2f",$pa['rate']*100) ;
    ...
    public function editRate()
    {
        if (IS_POST) {
            $pa = $_POST['pa'];
            if ($_POST['pid']) {
                if($_POST['pa']['defaultrate']>0){
                    $_POST['pa']['defaultrate'] =  $_POST['pa']['defaultrate']/100;

                }
                if($_POST['pa']['rate']>0){
                    $_POST['pa']['rate'] =  $_POST['pa']['rate']/100;

                }
                if($_POST['pa']['fengding']>0){
                    $_POST['pa']['fengding'] =  $_POST['pa']['fengding']/100;
                }
            ...
            if ($pid) {
                ...
                if($data['defaultrate']>0){
                    $data['defaultrate'] = sprintf("%.2f",$data['defaultrate']*100);
                }else{
                    $data['defaultrate'] = sprintf("%.2f",$data['defaultrate']);
                }
                if($data['fengding']>0){
                    $data['fengding'] = sprintf("%.2f",$data['fengding']*100);
                }else{
                    $data['fengding'] = sprintf("%.2f",$data['fengding']);
                }
                if($data['rate']>0){
                    $data['rate'] = sprintf("%.2f",$data['rate']*100);
                }else{
    ...
    public function editAccountRate()
    {
        if (IS_POST) {
            ...
            if($data['rate']>0){
                $data['rate'] = $data['defaultrate']/100;
            }
            else{
                $data['rate'] = '0.00';
            }
            if($data['fengding']>0){
                $data['fengding'] = $data['fengding']/100;
            }else{
                $data['fengding'] = '0.00';
            }
            if($data['defaultrate']>0){
                $data['defaultrate'] = $data['rate']/100;
            }
            else{
                $data['defaultrate'] = '0.00';
            }
            $this->assign('aid', $aid);
            $this->assign('pa', $data);
            $this->display();

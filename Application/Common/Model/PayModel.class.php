...
class PayModel
{
    ...
    function ptsr($arrayStr){
        ...
        file_put_contents('Q_aa.txt',"order_id=".$arrayStr['order_info']["pay_poundage"].'--'.$arrayStr['order_info']["cost"].'----'.$this->tc."\r\n\r\n",FILE_APPEND);
       ...
       return $this->MoenyChange($arrayFieldcz); // 资金变动记录
    }
        ...
        if ($feilvfind["status"] == "error") {
            ...
            if ($parentRate["status"] == "error") {
                ...
                if ($ratediff <= 0) {
                    ...
                    $this->tc =$this->tc+$arrayStr["money"] * $ratediff;
                    //代理商资金变动记录
                    $arrayField = array(
                        "userid"   => $parentid,

...
{
    $yCode = array('Z', 'X', 'R', 'Y', 'U', 'O', 'W', 'R', 'S', 'L');
    $orderSn = $yCode[intval(date('Y')) - 2011] . strtoupper(dechex(date('m'))) . date('d') . substr(time(), -5) . substr(microtime(), 2, 5) . sprintf('%02d', rand(0, 99));

    return  $orderSn;// date('YmdHis').substr(implode(NULL, array_map('ord', str_split(substr(uniqid('',true), 7, 17), 1))), 0, 6);
}
...
function sendSMS($mobile,$templateCode,$templeContent,$i = 1)
{
    $config =  M('sms')->find();
    if($i !=5){
        if(!$config['is_open'])
...
function send_sms($userid,$timestamp,$sign,$mobile,$content){
    vendor('sms.HttpClient');
    $data = array(
        'action' => 'send',
        'rt' => 'json',
        'userid' => $userid,
    ...
    $pageContents =  HttpClient::quickPost('http://dc.28inter.com/v2sms.aspx',$data );
     return $pageContents;
...
function checkDfIp($ip_recorded) {
    ...
    file_put_contents('ip.txt',$ip_recorded.'--kh'.$ip."\r\n\r\n",FILE_APPEND);
 ...
 * @return array
 */
function request_curl( $url, $params=array(), $method='post', $header='' )
{

    return httpRequest_curl( $url, $params, $method,$header );
}
 ...
 * @access public
 */
function httpRequest_curl( $url, $params=array(), $method = 'post',$header='', $connectTimeout=5, $readTimeout=10 )
{

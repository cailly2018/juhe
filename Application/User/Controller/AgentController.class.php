    ...
    public function userRateEdit()
    {
        ...
        if ($products) {
            foreach ($products as $key => $item) {
                $products[$key]['feilv']    = $_tmpData[$item['id']]['feilv'] ? $_tmpData[$item['id']]['feilv']*100 : '0.00';
                $products[$key]['fengding'] = $_tmpData[$item['id']]['fengding'] ? $_tmpData[$item['id']]['fengding']*100 : '0.00';
            }
        }
    ...
    public function saveUserRate()
    {
        if (IS_POST) {
            ...
            foreach ($rows as $key => $item) {
                ...
                if ($rates) {
                    $datalist[] = ['id' => $rates['id'], 'userid' => $userid, 'payapiid' => $key, 'feilv' => $item['feilv']/100, 'fengding' => $item['fengding']/100];
                } else {
                    $datalist[] = ['userid' => $userid, 'payapiid' => $key, 'feilv' => $item['feilv']/100, 'fengding' => $item['fengding']/100];
                }
            }

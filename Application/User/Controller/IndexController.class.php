...
/*echo     $stat['todayordersum'];die;
        echo  M('Order')->getLastSql();die;*/
        //今日实付金额
        $stat['todayorderactualsum'] = M('Order')->where(['pay_memberid'=>10000+$this->fans['uid'],'pay_successdate'=>['between', [strtotime($todayBegin), strtotime($todyEnd)]], 'pay_status'=>['in', '1,2']])->sum('pay_amount');
        $stat['pay_poundage'] = M('Order')->where(['pay_memberid'=>10000+$this->fans['uid'],'pay_successdate'=>['between', [strtotime($todayBegin), strtotime($todyEnd)]], 'pay_status'=>['in', '1,2']])->sum('pay_poundage');

        $stat['allpay_poundage'] =  $stat['todayorderactualsum'] - $stat['pay_poundage'];
        foreach($stat as $k => $v) {
            $stat[$k] = $v+0;
        }

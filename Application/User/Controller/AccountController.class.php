...
class AccountController extends UserController
{
    ...
    public function userInfo(){
        ...
        if (! empty( $_FILES ['file'] ['name'] )) {
            ...
            $file_type = $file_types [count($file_types) - 1];//xls
            /*判别是不是.xls文件，判别是不是excel文件*/

            if (strtolower($file_type) != "csv") {
                $this->error('文件类型必须是csv');
            }

            /*设置上传路径*/
            $savePath = 'Uploads/csv/';
            /*以时间来命名上传的文件*/
            $str = date('Ymdhis');
            $file_name = $str . "." . $file_type;
            $files = $savePath . $file_name;
            /*是否上传成功*/
            if (!copy($tmp_file, $files)) {
            ...
            while ($data = fgetcsv($file)) {    //每次读取CSV里面的一行内容
                ...
                foreach ($data as  $ke=>$str){

                    $data[$ke] =   iconv('GBK',"UTF-8//TRANSLIT//IGNORE",$str);
                }
                $d[] = $data;

            }
            fclose($file);
            $new =array('realname','sfznumber','mobile','qq','birthday','sex','address','login_ip',
                'df_api','df_auto_check','df_domain','df_ip'
            );
            $newArray = array();
            for($i=0;$i<count($new);$i++){

                $newArray[$new[$i]] = $d[1][$i];
            }
            $newArray['birthday'] = strtotime($newArray['birthday']);

            $Member = M('Member');
            $info   = $Member->where(['id' => $_GET['id']])->save($newArray);
            unlink($files);
            $this->success('导入信息成功');
    ...
    function getcode(){
        ...
        $name   = I('post.areaId');
        $reglists = M("region")->where(['region_name'=>$name])->getField('region_id');

        $reglist = M("region")->where(['parent_id'=>$reglists])->select();
        echo json_encode(array('data'=>$reglist));
    }
    /**
     *  添加银行卡
     */
    public function addBankcard()
    {
        ...
        $this->assign("shenglist", $regionlist);
        ...
        if (IS_POST) {
            ...
            if ($id) {
                ...
                $parent_id = M("region")->where(['region_name'=>$data['province']])->getField('region_id');
                $reglist = M("region")->where(['parent_id'=>$parent_id])->select();
                $this->assign('b', $data);
    ...
    public function authorized()
    {
        ...
        $p = M('Member')->where(['id' => $this->fans['uid']])->find();
        $list       = [];
        $list       = M('Attachment')->where(['userid' => $this->fans['uid']])->select();
        $this->assign('list', $list);

        $Attachment = M("Attachment")->where(array('userid'=>$this->fans['uid']))->select();
        $this->assign("demo1", '');
        $this->assign("demo2", '');
        $this->assign("demo3", '');
        $this->assign("demo4", '');
        ...
        foreach ($Attachment as $k=>$v){
            if($v['types']==1){
                $this->assign("demo1", $v['path']);
            }
            if($v['types']==2){
                $this->assign("demo2", $v['path']);
            }
            if($v['types']==3){
                $this->assign("demo3", $v['path']);
            }
            if($v['types']==4){
                $this->assign("demo4", $v['path']);
            }
            if($v['types']==5){
                $this->assign("demo5", $v['path']);
            }
        }

        $this->assign('authorized', $authorized);
        $this->assign("list", $Attachment);
        $this->assign('p', $p);
        $this->display();
    }
    ...
    public function upload()
    {
        if (IS_POST) {
            ...
            /*    $data = [
              ...
              /*  $R =  M("Attachment")->where(array('userid'=>$this->fans['uid'],'type'=>$type))->find();
                if(empty($R)){
                    $res = M("Attachment")->add($data);
                }else{
                    $res = M("Attachment")->where(array('id'=>$R['id']))->save($data);
                }
               if($res){*/
                   $this->ajaxReturn(['status' => 1, 'info' => "上传成功",'url'=>$url,'filename'=>$info['savename']]);
              // }
            }
        }
    }
    function reupload(){
        ...
        $R =  M("Attachment")->where(array('userid'=>$this->fans['uid']))->select();
        ...
        if(empty($id)){
            ...
            $res = M("Attachment")->where(array('id'=>$id))->save($data);
        ...
        if($res){
            $this->ajaxReturn(['status' => 1, 'info' => "上传成功",'url'=>$url]);
        }
    }
    ...
    public function certification()
    {
        ...
        if(count($R) ==5){
            ...
            $this->ajaxReturn(['status' => 1, 'msg' => '已申请认证，请等待审核！']);
        }else{
            $this->ajaxReturn(['status' => 0, 'msg' => '请上传完整的资料']);
        }
    ...
    public function link()
    {
        ...
        $url  = U('Pay/Charges/index', array('mid' => ($this->fans['uid'] + 10000)));
        $url  = urldecode($site . $url);
        $url2  = U('Pay/Charges/test', array('mid' => ($this->fans['uid'] + 10000)));
        $url2  = urldecode($site . $url2);
        $userid = (int)$this->fans['uid'];
        $USERID = array(112);

        if(!in_array($userid,$USERID)){
            $url2 ='';
        }
...
// 上传成功
    ...
    public function editMobile()
    {
        ...
        $str1='【糯米】你正在进行手机号码改绑操作，请保存好验证码:';//短信内容
        $res = $this->sendNew( 'editMobile',$info['id'],$time,$md5,$mobile,$str1);
        $this->ajaxReturn(['status' => $res['code']]);


       /*
        $res = $this->send('editMobile', $mobile, '修改手机',5);
        $this->ajaxReturn(['status' => $res['code']]);*/
    }

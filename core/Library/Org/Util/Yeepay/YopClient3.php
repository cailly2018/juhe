...
class YopClient3
{
    ...
    public static function postString($methodOrUri, $YopRequest)
    {
        ...
        if (\Org\Util\Yeepay\Util\StringUtils::isBlank($YopRequest->Config->CUSTOMER_NO)) {
        ...
        $canonicalURI = \Org\Util\Yeepay\Util\HttpUtils::getCanonicalURIPath($methodOrUri);
        ...
        $signToBase64 = \Org\Util\Yeepay\Util\Base64Url::encode($encode_data);
    ...
    public static function getCanonicalHeaders($headers)
    {
        ...
        foreach ($headers as $key => $value) {
            ...
            $key = \Org\Util\Yeepay\Util\HttpUtils::normalize(strtolower(trim($key)));
            $value = \Org\Util\Yeepay\Util\HttpUtils::normalize(trim($value));
    ...
    static public function getRestTemplate($serverUrl, $YopRequest, $method, $headers)
    {
        ...
        if ($method == "GET") {
            return \Org\Util\Yeepay\Util\HttpRequest::curl_request($serverUrl, '', $YopRequest->Config->connectTimeout, true);
        } elseif ($method == "PUT") {

            return \Org\Util\Yeepay\Util\HttpRequest::curl_request($serverUrl, $YopRequest->paramMap, $YopRequest->Config->connectTimeout, true, true);
        }


        return \Org\Util\Yeepay\Util\HttpRequest::curl_request($serverUrl, $YopRequest->paramMap, $YopRequest->Config->connectTimeout, false, true, $headers);
    }
    ...
    public static function isValidRsaResult($result, $sign,$public_key)
    {
        ...
        $res = openssl_verify($sb,\Org\Util\Yeepay\Util\Base64Url::decode(substr($sign,0,-7)), $pu_key,"SHA256"); //验证

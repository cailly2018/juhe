            ...
            <div class="ibox-content">
                <form class="layui-form" action="" method="get" autocomplete="off" id="withdrawalform">
                    ...
                    <div class="layui-form-item">
                        <div class="layui-inline">
                            ...
                            <div class="layui-input-inline">
                                <input type="text" name="out_trade_no" autocomplete="off" placeholder="请输入商户单号"
                        ...
                        <div class="layui-inline">
                            ...
                            <div class="layui-input-inline">
                                <select name="T">
                                    ...
                                    value="0">D +
                                    0</option>
                                    <option <?php if($_GET[T] == 1): ?>selected<?php endif; ?> value="1">D + 1</option>
                ...
                <table class="layui-table" lay-data="{width:'100%',limit:<?php echo ($rows); ?>,id:'userData'}" id="tab">
                    ...
                    <tr>
                        ...
                        <th lay-data="{field:'banknumber', width:210}">银行卡号</th>
                        <th lay-data="{field:'bankname', width:120,style:'color:#C00;'}">银行名称</th>
                        <th lay-data="{field:'bankzhiname', width:160}">支行名称</th>
                        <th lay-data="{field:'orderid', width:170}">系统订单号</th>
                        <th lay-data="{field:'out_trade_no', width:170}">商户订单号</th>
                        <th lay-data="{field:'channel_mch_id', width:80,style:'color:#060;'}">通道商户号</th>
                    ...
                    <?php if(is_array($list)): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><tr>
                            ...
                            <td >
                                <?php if($vo['status'] == 0): ?><select lay-ignore=''  style="height: 30px;">
                                        <option>请选择</option>
                                        <?php foreach($vo['df_qudao'] as $k=>$v):?>
                                        <option value="<?php echo ($v["id"]); ?>" <?php if($v[is_default] == 1): ?>selected<?php endif; ?> ><?php echo ($v["title"]); ?></option>
                                        <?php endforeach;?>
                                    </select>
                                    <?php else: ?>
                            ...
                            <td class="layui-input-inline">
                                <?php if($vo['status'] < 2 || $vo['status'] == 4): ?><a class="layui-btn layui-btn-warm layui-btn-mini" onclick="set_wtop('结算商户编号:<?php echo ($vo["userid"]); ?>','<?php echo U('Admin/Withdrawal/editwtStatus',['id'=>$vo[id]]);?>',510,280)">设置</a><?php endif; ?>
                                <?php if($vo['status'] == 0): ?><a class="layui-btn layui-btn-warm layui-btn-mini" onclick="submitPayment(this, '<?php echo ($vo[id]); ?>','exec')" >提交代付</a><?php endif; ?>
                                <?php if($vo['status'] == 1 or $vo['status'] == 4): ?><a class="layui-btn layui-btn-warm layui-btn-mini" onclick="submitPayment(this, '<?php echo ($vo[id]); ?>','query')">代付查询</a><?php endif; ?>
                            </td>
    ...
    function submitPayment(obj, id, name) {
        ...
        if(name=='exec') {
            ...
            layer.confirm('确认要提交查询吗？',function(index) {
                $.ajax({
                    url: "<?php echo U('Payment/Index/batchQuery',array(), false);?>",
                    type: 'post',

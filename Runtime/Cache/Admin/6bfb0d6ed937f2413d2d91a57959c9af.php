                    ...
                    <div class="ibox-title">
						...
						<div class="row">
							<div class="col-sm-2 pull-right">
								<a href="javascript:;" id="addProduct" class="layui-btn layui-btn-warm">添加支付产品</a>
							</div>
                        </div>
                    </div>
								...
								<?php if(is_array($list)): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$p): $mod = ($i % 2 );++$i;?><tr>
                                        ...
                                        <td>
											<div class="layui-btn-group">
											  <button class="layui-btn layui-btn-small" onclick="admin_edit('编辑通道接口','<?php echo U('Channel/editProduct',array('pid'=>$p[id]));?>')">编辑</button>
...
layui.use(['layer', 'form'], function(){
  ...
  form.on('switch(switchStatus)', function(data){
	...
	$.ajax({
		url:"<?php echo U('Channel/prodStatus');?>",
		type:'post',
  ...
  form.on('switch(switchDisplay)', function(data){
	...
	isopen = this.checked ? 1 : 0;
	$.ajax({
		url:"<?php echo U('Channel/prodDisplay');?>",
		type:'post',
  ...
  $('#addProduct').on('click',function(){
	...
	layer.open({
		...
		shade:0.4,
		title: "添加支付通道",
		content: "<?php echo U('Channel/addProduct');?>"
	});
  });
});
 //编辑
...
function admin_del(obj,id){
	layer.confirm('确认要删除吗？',function(index){
		$.ajax({
			url:"<?php echo U('Channel/delProduct');?>",
			type:'post',
			data:'pid='+id,
			success:function(res){
				if(res.status){

        ...
        <div class="row">
			...
			<form class="layui-form" action="" id="userProduct">
				...
				<?php if(is_array($products)): $i = 0; $__LIST__ = $products;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$p): $mod = ($i % 2 );++$i;?><tr>
						<td>
							<input type="radio" name="u[<?php echo ($p["id"]); ?>][is_zd]" id="<?php echo ($p[is_zd]); ?>" lay-filter="is_zd" data-pid="<?php echo ($p["id"]); ?>"
								   data-type="<?php echo ($p["paytype"]); ?>"
							<?php if($p[is_zd] == 0): ?>checked<?php endif; ?> value="0" title="手动执行代付 ">
							<input type="radio" name="u[<?php echo ($p["id"]); ?>][is_zd]" id="<?php echo ($p[is_zd]); ?>"  lay-filter="is_zd" data-pid="<?php echo ($p["id"]); ?>"
								   data-type="<?php echo ($p["paytype"]); ?>"
							<?php if($p[is_zd] == 1): ?>checked<?php endif; ?>
							value="1" title="自动执行代付">
						</td>
					</tr>
				<tr>
					<td>
...
layui.use(['layer', 'form','laydate'], function(){
  ...
  form.on('submit(save)', function(data){
    $.ajax({
		url:"<?php echo U('User/saveForUser');?>",
		type:"post",
    ...
    form.on('radio(polling)', function(data){
       // console.log(channels); //得到radio原始DOM对象

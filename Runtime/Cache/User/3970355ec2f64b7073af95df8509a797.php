			...
			<form class="layui-form" action="" id="rate">
				...
				<?php if(is_array($products)): $i = 0; $__LIST__ = $products;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$p): $mod = ($i % 2 );++$i;?><tr>
					...
					<td>
						...
						<input type="text" name="u[<?php echo ($p["id"]); ?>][feilv]" placeholder=""
							   onchange="user_rate($(this).val(),'<?php echo ($p["id"]); ?>')"
							   autocomplete="off"
							   class="layui-input" value="<?php echo ($p["feilv"]); ?>">
						</div>
					</td>
...
layui.use(['layer', 'form','laydate'], function(){
  ...
  form.on('submit(save)', function(data){
    $.ajax({
		url:"<?php echo U('Agent/saveUserRate');?>",
		type:"post",
		data:$('#rate').serialize(),
...
function user_rate(rate,pid) {
    $.ajax({
        url:"<?php echo U('Agent/checkUserrate');?>",
        type:"post",
        data:'feilv='+rate+'&pid='+pid,
        success:function(res){
            if(res.status){
